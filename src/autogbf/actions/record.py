        ...
        while True:
            for packet in self.page.listen.steps(timeout=1):
                ...
                match status:
                    ...
                    case Status.TURN_END:
                        status_type = "attack"
                        guard = []
                        for s in packet.response.body.get("status").get(
                            "is_guard_status"
                        ):
                            guard.append(s.get("is_guard_status"))
                        is_fa = self.page.run_js(
                            "stage.gGameStatus.auto_attack", as_expr=True
                        )
                    ...
                    case Status.SUMMON_RESULT | Status.SUMMON_RESULT_DIE:
                        ...
                        param = self.page.run_js(
                            "stage.gGameStatus.attackQueue.queue[0].param", as_expr=True
                        )
                        summon_id = (
                            int(param.get("summon_id")) + 1
                            if param.get("summon_id") != "supporter"
                            else 6
                        )
                    ...
                    case Status.ABILITY_RESULT | Status.ABILITY_RESULT_DIE:
                        ...
                        if is_fa:
                            ...
                            ele_ablity_class = self.page.run_js(
                                "stage.gGameStatus.$use_ability.context.firstElementChild",
                                as_expr=True,
                            ).attr("class")
                            user, ability_number = ele_ablity_class[-3:].split("-")
                            target = (
                                int(param.get("ability_aim_num")) + 1
                                if param.get("ability_aim_num") is not None
                                else 0
                            )
            ...
            if self.parent.record_stop:
                ...
                if self.parent.record_name:
                    with open(
                        f"./custom/{self.parent.record_name}.json",
                        "w",
                        encoding="utf-8",
                    ) as file:
